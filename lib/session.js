...
async function SaveCreds(sessionId) {
    ...
    // 2. Check if SESSION_ID is prefixed Base64 (e.g. PGWIZ~<base64> or MEGA~<base64>)
    if (cleanId.includes('~')) {
        const parts = cleanId.split('~');
        const candidateB64 = parts[1] || parts[0];
        try {
            const decodedStr = Buffer.from(candidateB64, 'base64').toString('utf8');
            if (decodedStr.startsWith('{') && decodedStr.endsWith('}')) {
                const parsed = JSON.parse(decodedStr);
                if (parsed.me && parsed.me.id) parsed.registered = true;
                const credsPath = path.join(sessionDir, 'creds.json');
                fs.writeFileSync(credsPath, JSON.stringify(parsed, null, 2));
                console.log('✅ Prefixed Base64 session credentials decoded and saved successfully');
                return;
            }
        } catch {}
    }

    // 3. Check if SESSION_ID is raw Base64 JSON
    ...
    // 4. Remote Download with retries, timeout, and browser headers (bypasses Cloudflare WAF on Koyeb/Render/Heroku)
    const credsPath = path.join(sessionDir, 'creds.json');
    ...
    let lastError = null;
    const maxRetries = hasExistingCreds ? 1 : 3;
    const requestTimeout = hasExistingCreds ? 4000 : 15000;
