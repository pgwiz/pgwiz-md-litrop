...
async function getOrIssueAuthKey() {
    ...
    // Rate-limit device dispatch to once every 5 minutes (300,000 ms)
    if (now - tempAuthSession.lastSentAt >= 300000) {
...
async function getPanelAccessCode(forceNew = false) {
    if (hasEnvAuthKey()) {
        return {
            type: 'env',
            key: getEnvAuthKey().trim(),
            expiresInSeconds: null
        };
    }

    const now = Date.now();
    if (forceNew || !tempAuthSession.key || now >= tempAuthSession.expiresAt) {
        tempAuthSession.key = crypto.randomBytes(3).toString('hex').toUpperCase();
        tempAuthSession.generatedAt = now;
        tempAuthSession.expiresAt = now + 15 * 60 * 1000;
        tempAuthSession.lastSentAt = now;
    }

    return {
        type: 'temp',
        key: tempAuthSession.key,
        expiresInSeconds: Math.max(0, Math.floor((tempAuthSession.expiresAt - now) / 1000)),
        lastSentAt: tempAuthSession.lastSentAt
    };
}
...
module.exports = { app, server, PORT, getOrIssueAuthKey, checkAuth, getPanelAccessCode };
