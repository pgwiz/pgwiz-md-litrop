...
async function useSQLiteAuthState() {
    ...
    if (fs.existsSync(credsPath)) {
        try {
            ...
            if (fileMeId && (!sqliteMeId || fileMeId !== sqliteMeId)) {
                ...
                creds = importCredsFromFile(true); // New account identity: clear keys
                ...
                creds = importCredsFromFile(false); // Same account: preserve keys!
            }
        } catch (e) {
    ...
    const saveCreds = () => {
        try {
            ...
            // Also keep session/creds.json on disk in sync with current session tokens
            try {
                const credsPath = path.join(__dirname, '..', 'session', 'creds.json');
