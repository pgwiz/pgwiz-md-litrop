...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    // Check if sender LID matches bot user LID
    if (sock && sock.user && sock.user.lid && cleanJid(sock.user.lid) === senderIdClean) {
        return true;
    }
    ...
    if (sock && chatId && chatId.endsWith('@g.us') && senderId.includes('@lid')) {
        try {
            ...
            if (participant) {
                ...
                const isRealOwner = Array.isArray(ownerSetting)
                    ? ownerSetting.some(num => cleanJid(num) === pRealIdClean)
                    : cleanJid(ownerSetting) === pRealIdClean;

                if (isRealOwner || await isSudo(participant.id)) {
                    return true;
                }
            }
        } catch (e) {
